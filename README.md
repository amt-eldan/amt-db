...
| חשבוניות ספק | `/invoices` | רשימת החשבוניות שהתקבלו, העלאת PDF (חילוץ ב-Claude) או הזנה ידנית, וצפייה במסמך המצורף |
| סיכום חודשי | `/monthly` | טבלת רווחיות לפי חודש, עריכת שורה + ייצוא CSV (נפתח נכון באקסל בעברית) |
