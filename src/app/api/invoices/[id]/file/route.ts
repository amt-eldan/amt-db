...
import { getSupplierInvoiceFile } from "@/db/queries";
import { requireSession } from "@/lib/require-session";

export const runtime = "nodejs"; // Buffer

/**
 * The stored invoice PDF, for viewing in the browser. Session-protected: the
 * bytes are business documents, so they never get a public URL and are never
...
export async function GET(
  ...
  const file = await getSupplierInvoiceFile(invoiceId);
  if (!file) {
  ...
  const name = encodeURIComponent(file.fileName ?? `invoice-${invoiceId}.pdf`);
