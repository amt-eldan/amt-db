...
/** Manual invoice upload: drag a PDF (or several) and Claude reads each one. */
export function UploadInvoiceButton() {
  ...
  async function uploadOne(file: File): Promise<{ ok: boolean; error?: string }> {
    ...
    let res: Response;
    try {
      res = await fetch("/api/invoices", { method: "POST", body });
    } catch {
    ...
    const amount = data?.amount ? ` — ${data.amount} ${data.currency ?? ""}`.trimEnd() : "";
    toast.success(`${data?.supplier ?? file.name} · ${data?.invoiceNumber ?? ""}${amount}`);
            ...
            <div key={`${p.name}-${i}`} className="flex flex-col gap-1">
              ...
              {p.status === "extracting" && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">מחלץ נתונים מהחשבונית...</p>
                </>
              )}
