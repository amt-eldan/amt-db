...
/**
 * Courier-invoice upload: Claude reads the document and every shipment on it, and
 * the result lands in the pending list — never straight into the ledger.
 */
export function UploadCourierInvoiceButton() {
  ...
  async function uploadOne(file: File): Promise<{ ok: boolean; error?: string }> {
    ...
    let res: Response;
    try {
      res = await fetch("/api/courier-invoices", { method: "POST", body });
    } catch {
      return { ok: false, error: "שליחת הקובץ נכשלה — בדוק את החיבור לרשת" };
    }

    // The proxy redirects an expired session to the /login HTML page, so the body
    // is not always JSON.
    let data: {
      error?: string;
      courier?: string;
  ...
  async function handleFiles(fileList: FileList | null) {
    ...
    for (const [index, file] of files.entries()) {
      ...
      if (result.ok) {
        ...
        // user can retry it or fall back to the manual form.
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        >
          ...
          {uploading ? "מעלה..." : "העלאת חשבונית בלדר"}
        </Button>
        <input
          ref={inputRef}
          type="file"
              ...
              {p.status === "extracting" && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    מחלץ את המשלוחים ומקשר אותם לשורות...
                  </p>
                </>
              )}
